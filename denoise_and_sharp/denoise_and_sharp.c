...
// ノイズ除去(gray: 元画像のデータ, denoise: ノイズ除去後の画像データ, width: 画像の幅, height: 画像の高さ)
void median_filter(unsigned char gray[], unsigned char denoise[], int width, int height) {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      ...
      // 近傍画素の値をソート
      qsort(neighbor, sizeof(neighbor) / sizeof(neighbor[0]), sizeof(int), compareInt);
  ...
  if ((img_denoise = fopen("denoise.pgm", "wb")) == NULL) exit(1);
...
// 鮮鋭化(denoise: ノイズ除去後の画像データ, width: 画像幅, height: 画像高さ)
void laplacian_filter(unsigned char denoise[], int width, int height) {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      ...
      if ((denoise[pos] - laplacian_sum) < 0) sharp[pos] = 0;
      else if ((denoise[pos] - laplacian_sum) > 255) sharp[pos] = 255;
      else sharp[pos] = denoise[pos] - laplacian_sum;
    }
  }

  // 書き込むファイルを開く(開けなかった場合プログラムを終了)
  if ((img_sharp = fopen("sharp.pgm", "wb")) == NULL) exit(1);
